...
async function logAction(action: string, entityId?: string, oldValues?: any, newValues?: any) {
  try { await adminApi.logAction(action, "order", entityId, oldValues, newValues); } catch {}
}
...
async function ensureInvoiceNumber(orderId: string, orderNumber: string): Promise<string> {
  const { data: order } = await db().from("orders").select("invoice_number").eq("id", orderId).maybeSingle();
  ...
  const { error } = await db().from("orders").update({ invoice_number: invoiceNumber }).eq("id", orderId);
  if (error) {
...
export const ordersApi = {
  async list(filters: {
    ...
    archived?: boolean; customerId?: string;
  } = {}) {
  ...
  async updatePaymentStatus(id: string, status: string): Promise<void> {
    const { data: oldOrder } = await db().from("orders").select("payment_status, total_amount").eq("id", id).maybeSingle();
  ...
  async deleteNote(id: string, noteTimestamp: string): Promise<void> {
    ...
    await db().from("orders").update({ notes: lines.join("\n"), updated_at: new Date().toISOString() }).eq("id", id);
    await logAction("note_deleted", id);
  },

  async updateTracking(id: string, data: {
    courier_name?: string; tracking_id?: string; tracking_url?: string;
    shipping_service?: string; estimated_delivery_at?: string;
    package_weight?: number; package_count?: number;
  }): Promise<void> {
    const { error } = await db().from("orders").update({
      ...data,
      updated_at: new Date().toISOString(),
  ...
  async duplicateOrder(id: string): Promise<string | null> {
    ...
    await logAction("order_duplicated", id, null, { new_order_id: newOrder.id, new_order_number: newOrderNumber });
    return newOrder.id;
  },
  ...
  async getCustomerSummary(customerId: string): Promise<{
    totalOrders: number; totalCompleted: number; totalCancelled: number;
    totalSpent: number; totalRefunded: number; lastOrderDate: string | null;
    customerSince: string | null;
  }> {
    const { data: customer } = await db().from("customers").select("*").eq("id", customerId).maybeSingle();
    if (!customer) return { totalOrders: 0, totalCompleted: 0, totalCancelled: 0, totalSpent: 0, totalRefunded: 0, lastOrderDate: null, customerSince: null };
    ...
    return {
      ...
      totalSpent: customer.total_spent || orderList.reduce((s: number, o: any) => s + (o.total_amount || 0), 0),
      totalRefunded: orderList.filter((o: any) => o.order_status === "refunded").reduce((s: number, o: any) => s + (o.total_amount || 0), 0),
  ...
  async createRefund(orderId: string, amount: number, reason: string): Promise<void> {
    const { data: order } = await db().from("orders").select("payment_status, total_amount").eq("id", orderId).maybeSingle();
    ...
    const totalPaid = (payments as any[] || []).reduce((s: number, p: any) => s + (p.status === "paid" ? Number(p.amount) : 0), 0);
    ...
    await logAction("refund_created", orderId, null, { amount, reason, payment_status: newPaymentStatus });
  },
