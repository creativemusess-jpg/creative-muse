...
function AdminCustomers() {
  ...
  return (
    <AdminLayout>
      ...
      <div className="mb-4 relative flex-1 max-w-sm">
        ...
        <input type="text" placeholder="Search customers..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full rounded-lg border border-gray-200 py-2 pl-10 pr-4 text-sm outline-none focus:border-[#9C544D]" />
      </div>
      {loading ? (
        <AdminLoading />
      ) : customers.length === 0 ? (
  ...
  <Link to="/admin/customers/$id" params={{ id: c.id }} className="hover:text-[#9C544D]">{c.full_name || "—"}</Link>
</td>
