    ...
    <AdminLayout>
      ...
      {showHistory && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowHistory(false)}>
          <div className="max-h-[70vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-6 shadow-lg" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={() => setShowHistory(false)} className="mt-4 w-full rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50">Close</button>
          </div>
        </div>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <DataTable
            ...
            columns={[
              ...
              { key: "quantity", label: "Stock", sortable: true, render: (val, row) => {
                ...
                if (val <= threshold) return <span className="font-semibold text-amber-600">{val}</span>;
                return <span className="font-semibold text-green-600">{val}</span>;
              }},
              { key: "threshold", label: "Min", render: (val) => <span className="text-xs text-gray-400">{val ?? 5}</span> },
            ]}
            actions={(row) => (
              <div className="flex items-center gap-1">
                <button onClick={() => { setAdjustProduct(row); setAdjustQty(row.quantity ?? 0); setAdjustReason(""); }}
                  className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-amber-600"><Minus className="h-4 w-4" /></button>
                <button onClick={() => loadHistory(row.id)}
                  className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-blue-500"><History className="h-4 w-4" /></button>
              </div>
            )}
          />
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="rounded-xl border border-gray-200 bg-white p-5">
            ...
            <div className="mt-4 space-y-3">
              {data.filter((p) => (p.quantity ?? 0) <= (p.threshold ?? 5)).map((p) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setAdjustProduct(null)}>
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-lg" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-4 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase">New Quantity</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase">Reason</label>
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => setAdjustProduct(null)} className="rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50">Cancel</button>
              <button onClick={handleAdjust} className="rounded-lg bg-[#1a1a2e] px-4 py-2 text-sm font-semibold text-white hover:bg-[#2d1b4e]">Save</button>
            </div>
          </div>
        </div>
      )}
