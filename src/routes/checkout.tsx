...
function CheckoutPage() {
  ...
  const [pincodeStatus, setPincodeStatus] = useState<
    "idle" | "checking" | "verified" | "not_found" | "unavailable" | "invalid"
  >("idle");
  const [pincodeMsg, setPincodeMsg] = useState("");
  const [pincodeLocations, setPincodeLocations] = useState<
    Array<{ locality: string; type?: string }>
  >([]);
  const [selectedLocality, setSelectedLocality] = useState("");
  const [stateConflict, setStateConflict] = useState<{ conflict: boolean; message?: string }>({
    conflict: false,
  });
  ...
  const deliveryStateCode =
    selectedStateCode || address.stateCode || getStateCodeByName(address.state) || "";
      ...
      if (addr.postalCode.length === 6) {
        lookupPincode(addr.postalCode).then((result) => {
          if (result.locations.length > 0) {
            ...
            setPincodeLocations(
              result.locations.map((l) => ({ locality: l.locality, type: l.postOfficeType })),
            );
  ...
  useEffect(() => {
    ...
    if (defaultAddress && !selectedAddressId && !editingAddressId) {
      selectSavedAddress(defaultAddress);
    }
  }, [defaultAddress, addressesLoading, selectedAddressId, editingAddressId, selectSavedAddress]);
  ...
  const handlePincodeChange = useCallback((value: string) => {
    ...
    pincodeTimerRef.current = setTimeout(async () => {
      ...
      if (stateCode) {
        ...
        const matchingCity = getCitiesByState(stateCode).find(
  ...
  const handleStateChange = useCallback(
    (code: string) => {
      ...
      if (currentCity) {
        const cityStillValid = getCitiesByState(code).some(
      ...
      if (pincodeStatus === "verified") {
        setStateConflict(
          detectStateConflict({ selectedStateCode: code, pincodeStateCode: deliveryStateCode }),
        );
      }
    },
      ...
      <PageShell>
        <div className="mx-auto max-w-[1200px] px-6 py-16">
          <div className="flex min-h-[40vh] flex-col items-center justify-center gap-4 text-center">
            ...
            <Link
              to="/shop"
              search={{ category: "", metal: "", minPrice: "", maxPrice: "", sort: "Featured" }}
  ...
  return (
    <PageShell>
      <div className="mx-auto max-w-[1200px] px-6 py-16">
        <div className="flex items-center justify-center gap-2 sm:gap-4 mb-10 text-xs sm:text-sm">
          {STEPS.map((step, i) => (
            <div key={step} className="flex items-center gap-2">
              <div
                className={`flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-full text-[11px] font-bold ${
                  i === 1
        ...
        <p className="text-[11px] font-semibold tracking-[0.24em] text-[#9C544D] uppercase">
          Checkout
        </p>
        <h1 className="font-display mt-2 text-[32px] font-semibold text-[#1a1a2e]">
          Delivery Details
        </h1>
        ...
        >
          <div className="space-y-8">
              ...
              <h2 className="font-display text-lg font-semibold text-[#1a1a2e] flex items-center gap-2">
                ...
                {addresses.length > 0 && (
                  <button
                    ...
                    className="ml-auto text-[11px] font-semibold text-[#9C544D] uppercase tracking-wider hover:underline"
                  >
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-start gap-3 rounded-xl border p-3 text-left text-sm transition-colors ${
                          isSelected
                            ? "border-[#9C544D] bg-[#fdf8f3] ring-1 ring-[#9C544D]/30"
                            : "border-[#e0d8cc] hover:border-[#9C544D]/50"
                        }`}
                      >
                        <button
                          type="button"
                        ...
                        >
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="text-[10px] text-[#7a6e64] flex items-center gap-1">
                              {addressTypeIcon(addr.addressType)}
                              {addr.addressType}
                            </span>
                            {addr.isDefault && (
                        ...
                        <div className="flex shrink-0 flex-col gap-1">
                          <button
                            ...
                            className="rounded-full p-1.5 text-gray-400 hover:bg-[#f5efe8] hover:text-[#9C544D] transition-colors"
                            title="Edit"
                          >
                          ...
                          {!addr.isDefault && (
                            <button
                              ...
                              className="rounded-full p-1.5 text-gray-400 hover:bg-[#f5efe8] hover:text-[#9C544D] transition-colors text-[9px] font-semibold uppercase tracking-wider"
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <div className="sm:col-span-2">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-[#e0d8cc] px-4 py-3 text-sm outline-none focus:border-[#9C544D]"
                  />
                </div>
                <div className="sm:col-span-2">
                  <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1">
                  ...
                  <div className="relative">
                    ...
                    <span className="absolute right-3 top-1/2 -translate-y-1/2">
                      {pincodeStatus === "checking" ? (
                        ...
                        <Check className="h-4 w-4 text-[#9C544D]" />
                      ) : pincodeStatus === "not_found" || pincodeStatus === "invalid" ? (
                  ...
                  {pincodeMsg && (
                    <p
                      className={`mt-1 text-[11px] font-medium ${pincodeStatus === "verified" ? "text-[#9C544D]" : pincodeStatus === "checking" ? "text-amber-600" : "text-red-600"}`}
                    >
                      {pincodeMsg}
                    </p>
                  )}
                  ...
                  <div className="relative">
                    {selectedStateCode ? (
                      <select
                        ...
                        className="w-full appearance-none rounded-xl border border-[#e0d8cc] px-4 py-3 pr-8 text-sm outline-none focus:border-[#9C544D] bg-white"
                      >
            ...
            <div className="rounded-[28px] bg-white p-6 shadow-[0_4px_24px_rgba(0,0,0,0.05)]">
              <h2 className="font-display text-lg font-semibold text-[#1a1a2e] flex items-center gap-2">
                <MapPin className="h-4 w-4 text-[#9C544D]" /> Billing Address
              </h2>
            ...
            <div className="rounded-[28px] bg-white p-6 shadow-[0_4px_24px_rgba(0,0,0,0.05)]">
              <h2 className="font-display text-lg font-semibold text-[#1a1a2e] flex items-center gap-2">
                <Truck className="h-4 w-4 text-[#9C544D]" /> Delivery Option
              </h2>
              <div className="mt-4 space-y-3">
                <label
                  className={`flex cursor-pointer items-center gap-3 rounded-xl border p-4 transition-colors ${deliveryMethod === "standard" ? "border-[#9C544D] bg-[#fdf8f3] ring-1 ring-[#9C544D]/30" : "border-[#e0d8cc] hover:border-[#9C544D]/50"}`}
                >
                  <input
                    type="radio"
                    name="delivery"
                    ...
                    className="h-4 w-4 accent-[#9C544D]"
                  />
                  <div className="flex-1">
                    <p className="font-medium text-[#1a1a2e]">Standard Insured Delivery</p>
                    <p className="text-xs text-[#7a6e64]">Free · 3–5 business days</p>
                  </div>
                  <span className="text-sm font-semibold text-[#9C544D]">Free</span>
                </label>
                <label
                  className={`flex cursor-pointer items-center gap-3 rounded-xl border p-4 transition-colors ${deliveryMethod === "express" ? "border-[#9C544D] bg-[#fdf8f3] ring-1 ring-[#9C544D]/30" : "border-[#e0d8cc] hover:border-[#9C544D]/50"}`}
                >
                  <input
                    type="radio"
                    name="delivery"
                  ...
                  />
                  <div className="flex-1">
                    <p className="font-medium text-[#1a1a2e]">Express Delivery</p>
            ...
            <div className="rounded-[28px] bg-white p-6 shadow-[0_4px_24px_rgba(0,0,0,0.05)]">
              ...
              <div className="mt-5 space-y-2 border-t border-[#e0d8cc] pt-4 text-sm">
                ...
                {totals.couponDiscount > 0 && (
                  <Row label="Coupon Discount" value={`-${formatPrice(totals.couponDiscount)}`} />
                )}
                {giftPackagingEnabled && giftCfg?.enabled && (
                  <Row
                ...
                {totals.shippingCharge > 0 && (
                  <p className="text-[10px] text-[#7a6e64] -mt-1">
                    {totals.deliveryLabel} · {totals.deliveryEstimate}
                  </p>
                )}
