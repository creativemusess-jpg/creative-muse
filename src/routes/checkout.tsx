  ...
  return (
    <PageShell>
      <div className="mx-auto max-w-[1200px] px-6 py-16">
          ...
          {STEPS.map((step, i) => (
            <div key={step} className="flex items-center gap-2">
              <div className={`flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-full text-[11px] font-bold ${
                i === 1 ? "bg-[#9C544D] text-white" : i < 1 ? "bg-[#9C544D] text-white" : "bg-gray-100 text-gray-400"
        ...
        <p className="text-[11px] font-semibold tracking-[0.24em] text-[#9C544D] uppercase">Checkout</p>
          ...
          <div className="space-y-8">
            <div className="rounded-[28px] bg-white p-6 shadow-[0_4px_24px_rgba(0,0,0,0.05)]">
              <h2 className="font-display text-lg font-semibold text-[#1a1a2e] flex items-center gap-2">
                <MapPin className="h-4 w-4 text-[#9C544D]" /> Delivery Address
                {addresses.length > 0 && (
                  <button type="button" onClick={() => setShowSavedAddresses(!showSavedAddresses)} className="ml-auto text-[11px] font-semibold text-[#9C544D] uppercase tracking-wider hover:underline">
              ...
              {showSavedAddresses && addresses.length > 0 && (
                <div className="mt-4 space-y-2 max-h-80 overflow-y-auto">
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-start gap-3 rounded-xl border p-3 text-left text-sm transition-colors ${
                          isSelected ? "border-[#9C544D] bg-[#fdf8f3] ring-1 ring-[#9C544D]/30" : "border-[#e0d8cc] hover:border-[#9C544D]/50"
                        }`}
                      >
                        <button type="button" onClick={() => selectSavedAddress(addr)} className="mt-1 shrink-0">
                          <div className={`flex h-4 w-4 items-center justify-center rounded-full border-2 ${
                            isSelected ? "border-[#9C544D] bg-[#9C544D]" : "border-gray-300"
                          }`}>
                        ...
                        <button type="button" onClick={() => selectSavedAddress(addr)} className="flex-1 text-left min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {addr.isDefault && (
                              <span className="inline-flex items-center gap-0.5 rounded-full bg-[#9C544D]/10 px-2 py-0.5 text-[10px] font-semibold text-[#9C544D] uppercase">
                        ...
                        <div className="flex shrink-0 flex-col gap-1">
                          <button type="button" onClick={() => handleEditAddress(addr)} className="rounded-full p-1.5 text-gray-400 hover:bg-[#f5efe8] hover:text-[#9C544D] transition-colors" title="Edit">
                          ...
                          {!addr.isDefault && (
                            <button type="button" onClick={() => handleSetDefault(addr.id)} className="rounded-full p-1.5 text-gray-400 hover:bg-[#f5efe8] hover:text-[#9C544D] transition-colors text-[9px] font-semibold uppercase tracking-wider" title="Set as Default">
                  ...
                  <button type="button" onClick={handleAddNew}
                    className="flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-[#9C544D]/40 p-3 text-sm font-medium text-[#9C544D] hover:bg-[#fdf8f3] transition-colors">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <div className="sm:col-span-2">
                  ...
                  <input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Your full name" className="w-full rounded-xl border border-[#e0d8cc] px-4 py-3 text-sm outline-none focus:border-[#9C544D]" />
                </div>
                <div className="sm:col-span-2">
                  ...
                  <div className="relative">
                    <input value={pincodeInput} onChange={(e) => handlePincodeChange(e.target.value)} placeholder="Enter 6-digit PIN code" maxLength={6}
                      className={`w-full rounded-xl border px-4 py-3 pr-10 text-sm outline-none transition-colors ${pincodeStatus === "verified" ? "border-[#9C544D] bg-[#fdf8f3]" : pincodeStatus === "checking" ? "border-amber-300 bg-amber-50" : pincodeStatus === "not_found" || pincodeStatus === "invalid" ? "border-red-300 bg-red-50" : "border-[#e0d8cc] focus:border-[#9C544D]"}`} />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2">
                      {pincodeStatus === "checking" ? <Loader2 className="h-4 w-4 animate-spin text-amber-500" /> : pincodeStatus === "verified" ? <Check className="h-4 w-4 text-[#9C544D]" /> : pincodeStatus === "not_found" || pincodeStatus === "invalid" ? <AlertCircle className="h-4 w-4 text-red-500" /> : null}
                    </span>
                  </div>
                  {pincodeMsg && <p className={`mt-1 text-[11px] font-medium ${pincodeStatus === "verified" ? "text-[#9C544D]" : pincodeStatus === "checking" ? "text-amber-600" : "text-red-600"}`}>{pincodeMsg}</p>}
                </div>
                <div>
                  ...
                  <div className="relative">
                    <select value={selectedStateCode} onChange={(e) => handleStateChange(e.target.value)}
                      className={`w-full appearance-none rounded-xl border px-4 py-3 pr-8 text-sm outline-none focus:border-[#9C544D] bg-white ${selectedStateCode ? "text-[#1a1a2e]" : "text-[#7a6e64]"}`}>
                ...
                <div>
                  ...
                  <div className="relative">
                    {selectedStateCode ? (
                      <select value={selectedCityId} onChange={(e) => handleCityChange(e.target.value)} className="w-full appearance-none rounded-xl border border-[#e0d8cc] px-4 py-3 pr-8 text-sm outline-none focus:border-[#9C544D] bg-white">
                        <option value="">Select city</option>
                        {cityOptions.map((c) => (<option key={c.id} value={c.id}>{c.name}</option>))}
                      </select>
                    ) : (
                      <input value={address.city} onChange={(e) => setAddress({ ...address, city: e.target.value })} placeholder="Select a state first" className="w-full rounded-xl border border-[#e0d8cc] px-4 py-3 text-sm outline-none focus:border-[#9C544D]" />
                    )}
                    <ChevronRight className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 rotate-90 text-[#7a6e64]" />
                  </div>
                </div>
                {pincodeLocations.length > 0 && (
                  <div className="sm:col-span-2">
                    ...
                    <div className="relative">
                      <select value={selectedLocality} onChange={(e) => handleLocalityChange(e.target.value)} className="w-full appearance-none rounded-xl border border-[#e0d8cc] px-4 py-3 pr-8 text-sm outline-none focus:border-[#9C544D] bg-white">
                ...
                <div className="sm:col-span-2">
                  ...
                  <input value={address.line1} onChange={(e) => setAddress({ ...address, line1: e.target.value })} placeholder="House / Shop / Building No., Street" className="w-full rounded-xl border border-[#e0d8cc] px-4 py-3 text-sm outline-none focus:border-[#9C544D]" />
                </div>
                <div className="sm:col-span-2">
                  <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1">Address Line 2</label>
                  <input value={address.line2} onChange={(e) => setAddress({ ...address, line2: e.target.value })} placeholder="Apartment, Suite, Unit (optional)" className="w-full rounded-xl border border-[#e0d8cc] px-4 py-3 text-sm outline-none focus:border-[#9C544D]" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1">Landmark</label>
                  <input value={address.landmark} onChange={(e) => setAddress({ ...address, landmark: e.target.value })} placeholder="Near… (optional)" className="w-full rounded-xl border border-[#e0d8cc] px-4 py-3 text-sm outline-none focus:border-[#9C544D]" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1">Phone *</label>
                  <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder={user.email} className="w-full rounded-xl border border-[#e0d8cc] px-4 py-3 text-sm outline-none focus:border-[#9C544D]" />
                </div>
              </div>

              <label className="mt-4 flex items-center gap-2 text-xs text-[#7a6e64]">
                <input type="checkbox" checked={saveAddr} onChange={(e) => setSaveAddr(e.target.checked)} className="accent-[#9C544D]" />
                Save this address for future orders
              </label>
            </div>

            <div className="rounded-[28px] bg-white p-6 shadow-[0_4px_24px_rgba(0,0,0,0.05)]">
              <h2 className="font-display text-lg font-semibold text-[#1a1a2e] flex items-center gap-2"><MapPin className="h-4 w-4 text-[#9C544D]" /> Billing Address</h2>
              <label className="mt-3 flex items-center gap-2 text-sm">
                <input type="checkbox" checked={billingSame} onChange={(e) => setBillingSame(e.target.checked)} className="accent-[#9C544D]" />
              ...
              {!billingSame && (
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  <div className="sm:col-span-2">
                    ...
                    <input value={billingAddress.line1} onChange={(e) => setBillingAddress({ ...billingAddress, line1: e.target.value })} className="w-full rounded-xl border border-[#e0d8cc] px-4 py-3 text-sm outline-none focus:border-[#9C544D]" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1">Billing Address Line 2</label>
                    <input value={billingAddress.line2} onChange={(e) => setBillingAddress({ ...billingAddress, line2: e.target.value })} className="w-full rounded-xl border border-[#e0d8cc] px-4 py-3 text-sm outline-none focus:border-[#9C544D]" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1">City *</label>
                    <input value={billingAddress.city} onChange={(e) => setBillingAddress({ ...billingAddress, city: e.target.value })} className="w-full rounded-xl border border-[#e0d8cc] px-4 py-3 text-sm outline-none focus:border-[#9C544D]" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1">State *</label>
                    <input value={billingAddress.state} onChange={(e) => setBillingAddress({ ...billingAddress, state: e.target.value })} className="w-full rounded-xl border border-[#e0d8cc] px-4 py-3 text-sm outline-none focus:border-[#9C544D]" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1">Postal Code *</label>
                    <input value={billingAddress.postalCode} onChange={(e) => setBillingAddress({ ...billingAddress, postalCode: e.target.value })} className="w-full rounded-xl border border-[#e0d8cc] px-4 py-3 text-sm outline-none focus:border-[#9C544D]" />
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="rounded-[28px] bg-white p-6 shadow-[0_4px_24px_rgba(0,0,0,0.05)]">
              <h2 className="font-display text-lg font-semibold text-[#1a1a2e] flex items-center gap-2"><Truck className="h-4 w-4 text-[#9C544D]" /> Delivery Option</h2>
              <div className="mt-4 space-y-3">
                <label className={`flex cursor-pointer items-center gap-3 rounded-xl border p-4 transition-colors ${deliveryMethod === "standard" ? "border-[#9C544D] bg-[#fdf8f3] ring-1 ring-[#9C544D]/30" : "border-[#e0d8cc] hover:border-[#9C544D]/50"}`}>
                  <input type="radio" name="delivery" value="standard" checked={deliveryMethod === "standard"} onChange={(e) => setDeliveryMethod(e.target.value as DeliveryMethod)} className="h-4 w-4 accent-[#9C544D]" />
                  ...
                  <span className="text-sm font-semibold text-[#9C544D]">Free</span>
                </label>
                <label className={`flex cursor-pointer items-center gap-3 rounded-xl border p-4 transition-colors ${deliveryMethod === "express" ? "border-[#9C544D] bg-[#fdf8f3] ring-1 ring-[#9C544D]/30" : "border-[#e0d8cc] hover:border-[#9C544D]/50"}`}>
                  <input type="radio" name="delivery" value="express" checked={deliveryMethod === "express"} onChange={(e) => setDeliveryMethod(e.target.value as DeliveryMethod)} className="h-4 w-4 accent-[#9C544D]" />
