...
function PaymentPage() {
  ...
  useEffect(() => {
    if (!authLoading && cartReady && !checkoutData && lines.length === 0) {
      navigate({ to: "/cart" });
    }
  ...
  const giftPackPrice =
    checkoutData.giftPackagingEnabled && giftCfg?.enabled ? giftCfg?.price || 0 : 0;
  ...
  const validateMethod = (): string | null => {
    ...
    if (method === "card" && cardNumber.replace(/\s/g, "").length < 13)
    ...
    if (method === "cod" && totalAmount > COD_MAX_AMOUNT)
      return "COD is not available for orders above ₹50,000.";
    return null;
  };
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const result = await createOrder({
        ...
        giftPackagingName:
          checkoutData.giftPackagingEnabled && giftCfg?.enabled
            ? giftCfg?.name || "Gift Packaging"
            : "",
      ...
      <div className="mx-auto max-w-[1200px] px-6 py-16">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden items-center gap-2 rounded-full border border-amber-200 bg-amber-50 px-4 py-2 text-xs font-medium text-amber-700 sm:flex">
            <ShieldCheck className="h-4 w-4" /> Secure Demo Checkout — No real payment will be
            charged.
          </div>
        </div>
        ...
        <div className="mt-2 flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs font-medium text-amber-700 sm:hidden">
          <ShieldCheck className="h-4 w-4 shrink-0" /> Secure Demo Checkout — No real payment will
          be charged.
        </div>
        ...
        {retryCount > 0 && (
          <div className="mt-4 flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-700">
            <AlertCircle className="h-4 w-4 shrink-0" /> Payment failed. You can review your
                ...
                {method === "upi" && (
                  <div className="space-y-4">
                    ...
                    <p className="text-xs text-[#7a6e64]">
                      Demo: Enter any valid UPI ID format (e.g., name@upi)
                    </p>
                  </div>
                )}
                {method === "card" && (
                  <div className="space-y-4">
                    ...
                    <p className="text-xs text-[#7a6e64]">
                      Demo: Use 4111 1111 1111 1111 for testing.
                    </p>
                  </div>
                )}
                {method === "netbanking" && (
                  <div className="space-y-4">
                    ...
                    <p className="text-xs text-[#7a6e64]">
                      Demo: Selecting a bank simulates the payment.
                    </p>
                  </div>
                )}
                {method === "wallet" && (
                  <div className="space-y-4">
                    <div className="flex flex-wrap gap-3">
                      {["Paytm", "Amazon Pay", "Mobikwik", "Freecharge"].map((w) => (
                        <button
                          ...
                          className={`rounded-xl border px-5 py-3 text-sm font-medium transition-colors ${selectedWallet === w ? "border-[#9C544D] bg-[#9C544D] text-white shadow-[0_6px_16px_rgba(156,84,77,0.18)]" : "border-[#e0d8cc] text-[#7a6e64] hover:border-[#9C544D]/50"}`}
                        >
                          {w}
                        </button>
                      ))}
                    ...
                    <p className="text-xs text-[#7a6e64]">
                      Demo: No real wallet authentication is performed.
                    </p>
                  </div>
                )}
                {method === "cod" && (
                  <div className="space-y-4">
                    ...
                    >
                      ...
                      <p className="mt-1 text-xs text-[#7a6e64]">
                        {codEnabled
                          ? "Pay when your jewellery arrives. Available in eligible areas."
                          : `COD is not available for orders above ₹${COD_MAX_AMOUNT.toLocaleString("en-IN")}.`}
                      </p>
                    </div>
                  </div>
                )}
            ...
            <div className="rounded-[28px] bg-white p-6 shadow-[0_4px_24px_rgba(0,0,0,0.05)]">
                ...
                {checkoutData.items.map((item: any) => (
                  <div key={item.productId} className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-xs font-medium text-[#1a1a2e]">{item.name}</p>
                      <p className="text-[10px] text-[#7a6e64]">×{item.qty}</p>
                    </div>
              ...
              <div className="mt-4 space-y-2 text-sm">
                ...
                {totals.discountAmount > 0 && (
                  <Row label="Discount" value={`-${formatPrice(totals.discountAmount)}`} />
                )}
                <Row
                  label="Shipping"
              ...
              <div className="mt-4 rounded-xl bg-[#fdf8f3] p-3 text-xs text-[#7a6e64]">
                <p>
                  ...
                  {checkoutData.address.state} {checkoutData.address.postalCode}
                </p>
                {checkoutData.deliveryMethod === "express" && (
                  <p className="mt-1 text-purple-600 font-medium">Express Delivery</p>
                )}
              </div>
