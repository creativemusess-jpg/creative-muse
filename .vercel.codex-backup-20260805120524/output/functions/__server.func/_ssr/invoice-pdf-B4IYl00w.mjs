...
//#region node_modules/.nitro/vite/services/ssr/assets/invoice-pdf-B4IYl00w.js
var import_jspdf_node_min = /* @__PURE__ */ __toESM(require_jspdf_node_min());
var money = (value) => `Rs. ${Number(value || 0).toLocaleString("en-IN")}`;
...
function addressLines(addr, fallbackName) {
	if (!addr) return ["-"];
...
async function generateInvoicePdf({ order, items, invoiceNumber }) {
	...
	await Promise.all(lines.map(async (item, index) => {
	...
	addressLines(order.delivery_address || order.shipping_address, order.customer_name).slice(0, 6).forEach((line, i) => doc.text(String(line), 108, y + 6 + i * 5));
	y = 92;
	autoTable(doc, {
		startY: y,
		margin: {
	...
	doc.setFont("helvetica", "normal");
	doc.text("Prices are inclusive of applicable taxes unless stated otherwise. Returns and exchanges are subject to store policy.", margin, 273);
	doc.text("Support: hello@creativemuse.in | Website: creativemuse.in | Phone: +91 90337 79867", margin, 278);
	doc.text("Authorized Signature", pageWidth - margin, 278, { align: "right" });
	doc.line(pageWidth - 58, 271, pageWidth - margin, 271);
	doc.save(`${inv}.pdf`);
}
//#endregion
export { generateInvoicePdf as t };
